   ...
   (defn -main
     ...
     (let [argv (vec argv)
           methods-dir (if (and *file* (not= *file* "NO_SOURCE_PATH"))
                         ...
                         (clojure.java.io/file "src" "masago" "methods"))
           repository-root (.. methods-dir getParentFile getParentFile getParentFile)
           seed (if (and (seq argv) (not (str/starts-with? (first argv) "--")))
                  (clojure.java.io/file (first argv))
                  (clojure.java.io/file repository-root "data" "seed-open-materials-graph.kotoba.edn"))

(ns masago.methods.social
  "social.cljc — masago DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from masago's own
...
(defn draft-observation-post
  "A dry-run observation post over masago's own domain (Open materials-discovery (公開材料) Knowledge-Graph mirror — the materials-science sibling of the public-scientific-data mirror lineage (rasen…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "masago R0: live social posting is Council Lv6+ + operator + "
